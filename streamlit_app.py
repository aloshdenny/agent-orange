...
# User inputs
prompt = st.text_area("Enter your writing prompt:", "write a book chapter on the AI implications in cardiovascular imaging")
...
if st.button("Start Writing"):
    ...
    api_keys = [
        ...
        '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',
        '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'
    ]
    st.session_state.master_agent = MasterAgent(model_id='llama-3.1-70b-versatile', api_key=api_keys[0])
    
    if not st.session_state.master_agent.use_chroma:
        st.warning("ChromaDB initialization failed. Falling back to in-memory storage.")
    ...
    for iteration in range(num_iterations):
        ...
        # Display relevant past memories
        relevant_memories = st.session_state.master_agent.get_relevant_memories(st.session_state.master_agent.project_memory)
        with st.expander("Relevant Past Memories:", expanded=False):
